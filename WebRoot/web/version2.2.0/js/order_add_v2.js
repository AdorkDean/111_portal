...
function searchCity(){
    ...
    if($.trim(search_input_id)==''){
      ...
      $.ajax({
			url:'/member/order!searchCity.action',
...
function checkReceiver() {
	...
	if (tmemberreceiver_receiver == '') {
		$("#receiverSpanFont").html("请填写收货人姓名，不超过10个字");
		flag = false;
	} else {
		$("#receiverSpanFont").html("");
	}
	...
	if($("#tmemberreceiver_areaId").val()==''){
		if (memberReceiver_adcode == ''||tmemberreceiver_longitude=='') {
			$("#weizhixinxi_span").html("请填写完整的位置信息");
			flag = false;
		} else {
	...
	if (tmemberreceiver_address == '') {
		$("#addressSpanFont").html("请填写收货人详细地址，不超过40个字");
		flag = false;
	} else {
		$("#addressSpanFont").html("");
	}

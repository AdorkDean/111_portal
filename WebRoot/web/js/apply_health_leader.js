var selectPro=0;//选择省市区时,IE下,非正常关闭处理
/**
 * 图片上传显示效果
 */
$(function(){
...
//		}
		...
		$(this).attr('style',"background-color:#B5B5B5");
		//$(this).val("提交中...");
		submiting(this);
...
function timeDesc(){
	...
	if(count>0){
		count--;
		$("#sendCode").val(count+"秒重新获取");
		$("#sendCode").prop("disabled", "disabled");
	}else{
		$("#sendCode").val("获取验证码");
		$("#sendCode").prop("disabled", "");
		count = 61;
		clearInterval(timer);
		timer = "";
	}
